...
class CassandraJolokiaCollector(JolokiaCollector):
    ...
    def clean_up(self, text):
        ...
        # The metric name as it comes in has two main parts separated by a
        # colon.
        s = string.split(text, ':')

        # We don't care about the first part, but the second part contains all
        # of the JMX keys.
        jmx_keys = s[1]
        ...
        if (-1 != last_comma_index):
                ...
                # and one of the other keys has dots in it, then we won't parse
                # things correctly.)
            last_dot_index = string.rfind(jmx_keys, '.', last_comma_index)
            # If there wasn't a dot, this just means that this metric didn't
            # have a statistic on the end
            if (-1 != last_dot_index):
                # If there was a dot, parse out the statistic, and strip it off
                # the jmx_keys
                statistic = jmx_keys[last_dot_index + 1:]
                jmx_keys = jmx_keys[:last_dot_index]

        # Parse the jmx_keys string to split everything out into an array of
        # name-value pairs.
        kvps = string.split(jmx_keys, ',')

        # Initialize the variables that will be used to hold the values from
        # the JXM keys
        m_type = ''
        keyspace = ''
        name = ''
        ...
        for i in range(len(kvps)):
            # Split the pair on the equal sign such that kvp[0] is the key and
            # kvp[1] is the value
            kvp = string.split(kvps[i], '=')
            # Replace any dots in the values with dashes
            kvp[1] = string.replace(kvp[1], '.', '-')
            # Check the name of the key, and set the appropriate variable
            # accordingly
            if (kvp[0].lower() == 'type'):
                m_type = kvp[1]
        ...
        # Now we need to build the metric name, which happens in slightly
        # different ways depending on the metric type.
        ...
        # 2) An aggregate across all tables and keyspaces in the Cassandra
        # node.
        if (m_type == 'ColumnFamily') or (m_type == 'ColumnFamilies'):
            if (keyspace != ''):
                # If the keyspace is not blank, it's a metric for a specific
                # keyspace and table
        ...
        # node.
            ...
            if (keyspace != ''):
                # If the keyspace is not blank, it's an aggrgate metric for a
                # specific keyspace
                metric_name = m_type + '._Keyspaces' + keyspace + scope + name
            else:
                # Otherwise, it's a global aggregate
                metric_name = 'Keyspace' + scope + name
        # If the metric is a thread pool metric, there is an additional 'path'
        # component to add to the metric name
