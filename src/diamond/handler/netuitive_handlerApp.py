...
# Configuration
Enable handler
  * handlers = diamond.handler.netuitive_cloud.NetuitiveHandlerApp
...
class NetuitiveHandlerApp(Handler):
    ...
    def __init__(self, config=None):
        ...
        try:
            ...
            logging.info("initialize Netuitive handlerApp")

            self.version = self._get_version()
            self.api = netuitive.Client(self.config['url_new'], self.config[
                                        'api_key_new'], self.version)
            ...
            logging.info('NetuitiveHandlerApp', self.config)

        except Exception as e:
            logging.exception('NetuitiveHandlerApp: init - %s', str(e))
    ...
    def get_default_config(self):
        ...
        config.update({
            'url_new': 'https://api.app.netuitive.com/ingest/infrastructure',
            'api_key_new': 'apikey',
            'tags': None,
            'relations': None,
            'location': None,
    ...
    def process(self, metric):
        ...
        if len(self.element.samples) >= self.batch_size:
            logging.info('flushing data due to exceeding batch_size')
            self.flush()

    def flush(self):
        logging.info('sending data to App')
        ...
        try:
            ...
            if len(self.element.metrics) >= (
                ...
                logging.warn('NetuitiveHandlerApp: Trimming backlog. Removing' +
            ...
            if elapsed > 900 or self.flush_time == 0:
                ...
                if self.api.disabled is False:
                    logging.info('NetuitiveHandlerApp: Data posted successfully. ' +
                                 'Next log message in 15 minutes.')

        except urllib2.HTTPError as e:
            if e.code in self.api.kill_codes:
                logging.exception('NetuitiveHandlerApp: flush - %s', str(e))
